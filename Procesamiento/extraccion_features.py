...
"""
Toma las entrevistas y genera el archivo   features_documentos_nuevo.json con todos los laS PALABRAS
"""
...
dic = pd.read_csv(pathapp + 'data/csv/DICCIONARIO.csv', sep=';')
listan = []
...
df = pd.read_csv(pathapp + "data/csv/DivisionPoliticoAdministrativaColombia.csv", sep=',')
...
with open( pathapp + "data/csv/alias_conocidos.txt") as f:
	texto_alias = f.read()
...
with open(pathapp +"/data/csv/conocidos.txt") as f:
...
def archivo_JSON(archivo):
	with open(pathapp + "/data/txtlimpios/"+archivo, 'r') as k:
		texto = k.read()
	js={ 
...
with open(pathapp +"/data/json/features_documentos_nuevo.json", 'w') as json_file:
