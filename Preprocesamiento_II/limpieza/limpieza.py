...
def words(text):
	...
	"""
	return re.findall(u'[\xf1\xe1\xe9\xed\xf3\xfaa-z]+', str(text))
...
def limpiar_linea(sentence):
	...
	linea_limpia = linea_limpia.replace(" , ", ",").replace(" . ", ".").replace(" ¿ ", "¿").replace(" ? ", "?").replace(" ( ", "(").replace(" ) ", ")")
	linea_limpia = linea_limpia.replace(",", " , ").replace(".", " . ").replace("¿", " ¿ ").replace("?", " ? ").replace("(", " ( ").replace(")", " ) ")
	linea_corregida = " ".join([correct(x) for x in linea_limpia.split()])
	linea_corregida = linea_corregida.replace(" , ", ",").replace(" . ", ".").replace(" ¿ ", "¿").replace(" ? ", "?").replace(" ( ", "(").replace(" ) ", ")")
	return linea_corregida

with open("nombres_escogidos.txt", 'r') as f:
	lines = f.readlines()
	nombres_archivos = [x.strip() for x in lines if len(x.strip()) > 0]
	for line in nombres_archivos:
		try:
			with open("data/" + line, 'r') as fread:
				lineas_texto = [x.strip() for x in fread.readlines() if len(x.strip()) > 0]
			with open("clean_data/" + line, 'a') as fwrite:
				for linea in lineas_texto:
					print(limpiar_linea(linea), file=fwrite)
		except:
			continue
